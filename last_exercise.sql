...
-- Esse Comando está dando erro no meu MySQL:
-- Error Code: 1064. You have an error in your SQL syntax; 
-- check the manual that corresponds to your MariaDB
-- server version for the right syntax to use near 
-- 'FULL OUTER JOIN subscriptions s ON c.client_id = s.client_id FULL OUTER JOIN ...' at line 3
SELECT c.client_name, m.movie_title, s.subscription_id, s.subscription_date
FROM clients c
FULL OUTER JOIN subscriptions s ON c.client_id = s.client_id
FULL OUTER JOIN movies m ON m.movie_id = s.movie_id;

SELECT c.client_name, m.movie_title
FROM clients c
CROSS JOIN movies m;